...
def printTable(merged, dataframes):
    ...
    for df in all_dataframes:
        ...
        textstr = format_table_string(
            [(str(round(b, 1)) + "%", str(a)) for a, b in top.values], delimiter=" "
        )
        row.append(textstr)
    table.add_row(row)
    ...
    for df in all_dataframes:
        ...
        textstr = format_table_string(
            [(str(round(b, 1)) + "%", str(a)) for a, b in d.items()], delimiter=" "
        )
        row.append(textstr)
    table.add_row(row)

    print(table)
    with open("tmp/overlap_result.txt", "w", encoding="utf-8", errors="ignore") as w:
        w.write(str(table))
